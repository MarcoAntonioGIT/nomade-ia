  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Simular verificação de autenticação (você pode substituir por lógica real)
        const isLoggedIn = localStorage.getItem('userLoggedIn') === 'true';
        
        if (!isLoggedIn) {
          toast({
            title: "Login necessário",
            description: "Faça login para visualizar seu roteiro personalizado.",
          });
          onAuthRequired();
        } else {
          // Passar dados para o componente pai
          onTripGenerated(formData, responseData.packages || []);
          toast({
            title: "Roteiro gerado com sucesso!",
            description: "Confira as opções de pacotes personalizadas para você.",
          });
        }
      } else {
